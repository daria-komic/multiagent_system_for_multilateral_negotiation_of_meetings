...
class UserAgentThree(agent.Agent):    
    ...
    def preference_function(self, start_time, end_time, date, difference_start, difference_end, meeting_duration, meeting_type):
        if self.preference == True:
            preferences = {"time" : 0.15, "date" : 0.3, "difference" : 0.05, "duration": 0.2, "week_day" : 0.2, "type": 0.1}
            ...
            if date == "2024-09-03":
                date_score = 2
            elif date == "2024-09-04":
                date_score = 2
            elif date == "2024-09-06":
                date_score = 2        
            else:
                date_score = 10    

            if difference_start == five_minutes or difference_end == five_minutes:
                difference_score = 10
            elif difference_start == ten_minutes or difference_end == ten_minutes:
                difference_score = 10    
            else:
                difference_score = 2
    ...
    class AvailabilityBehaviour(behaviour.CyclicBehaviour):
        ...
        async def run(self):
            with open('user3_calendar.json', 'r') as json_file:
                ...
                if msg.metadata['performative'] == 'inform':
                    try:
                        ...
                        if response and points:
                            response_message = "accept" if available and response == "accept" else "reject"  
                            ...
                            response_message = "accept" if available else "reject"
