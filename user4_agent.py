...
class UserAgentFour(agent.Agent):    
    ...
    def preference_function(self, start_time, end_time, date, difference_start, difference_end, meeting_duration, meeting_type):
        if self.preference == True:
            preferences = {"time" : 0.15, "date" : 0.1, "difference" : 0.20, "duration": 0.15, "week_day" : 0.1, "type": 0.30}
            ...
            if difference_start == five_minutes or difference_end == five_minutes:
                difference_score = 2
            if difference_start == ten_minutes or difference_end == ten_minutes:
                difference_score = 2    
            else:
                difference_score = 9
    ...
    class AvailabilityBehaviour(behaviour.CyclicBehaviour):
        ...
        async def run(self):
            with open('user4_calendar.json', 'r') as json_file:
